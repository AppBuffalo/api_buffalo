class PhotosController < ApplicationController
  ...
  param :latitude, Float, required: true, desc: "Photo's latitude"
  param :longitude, Float, required: true, desc: "Photo's longitude"
  ...
  def create
    json = if params.include?(:user_id) && params.include?(:lat) && params.include?(:long) && params.include?(:s3_url) &&
              !params[:user_id].nil? && !params[:lat].nil? && !params[:long].nil? && !params[:s3_url].nil?
             if photo = Photo.create(user_id: params[:user_id], latitude: params[:lat],
                                     longitude: params[:long], s3_url: params[:s3_url])
               { id: photo.id }
             else
               { id: nil }
             end
  ...
  example "{id: 5, user_id: 12, latitude: -5.2, longitude: 5.65, s3_url: 'http://s3.com/photo.png'}"
  def show
  ...
  def index
    json = if params.include?(:user_id) && params.include?(:latitude) && params.include?(:longitude) &&
             ...
             photos = if params.include?(:page) && !params[:page].nil?
                        Photo.where.not(user_id: params[:user_id])
                            .near([params[:latitude], params[:longitude]])
                            .limit(10).offset(params[:page] * 10)
                      else
                        Photo.where.not(user_id: params[:user_id]).near([params[:latitude], params[:longitude]])
                      end
             photos.to_json
           else
             {}
